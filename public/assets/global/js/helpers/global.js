...
function previewImageFromInput(input, selector) {
...
function dateTimeShortMonthIndo(input) {
    let date;

    if (typeof input === 'string' && !isNaN(Date.parse(input))) {
        date = new Date(input);
    } else if (!isNaN(input)) {
        date = new Date(parseInt(input) * 1000); // asumsikan timestamp dalam detik
    } else {
        return 'Invalid datetime value';
    }

    const day = ('0' + date.getDate()).slice(-2);
    const month = getBulan([date.getMonth() + 1], true);
    ...
    // Deteksi apakah input string mengandung jam
    const hasTime = typeof input === 'string' && input.includes(':');
